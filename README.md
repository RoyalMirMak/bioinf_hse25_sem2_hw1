...
Выполнил Махлин Мирон. Весь код к заданию может быть найден в файле `main.ipynb`.
...
**Observed** \
Количество обнаруженных родов варьируется в диапазоне 65-80, что указывает на высокое богатство организмов обоих типов образцов. При этом 
высота второго ящика меньше, т.е. количество родов меньше варьируется.

**Shannon** \
Индекс Шеннона в среднем равен примерно 3.0, что свидетельствует о многокомпонентном сообществе.

**Simpson** \
Индекс Симпсона близок к 0.9 , что подтверждает высокую выравненность сообщества: виды представлены относительно равномерно, а не подавлены одним доминантом.

**Статистический анализ** \
Для оценки значимости различий между группами по Param1 был проведен тест Уилкоксона.

- Observed : p-value = 0.9378 > 0.05 (различия не значимы)
- Shannon : p-value = 0.6593 > 0.05 (различия не значимы)
- Simpson : p-value = 0.456 > 0.05 (различия не значимы)

### Задание 4

![beta_diversity](beta_diversity_pcoa.png)

Матрица Брея-Кертиса описывает насколько образцы отличаются друг от друга, опираясь на состав групп (смотрится отличие в разнообразии). Далее при помощи PCA матрица конвертируется в матрицу размером Nx2, которую уже можно изобразить и оценить, насколько похожи образцы разных типов между собой и как они кластеризуются.

Данных немного, однако их достаточно, чтобы увидеть кластеры по типам образцов. Синие точки сгруппированы по центру, в то время как оранжевые -- в углах и разделены синими. Значит, синяя группа (Variable 2) более стабильна и однородна, а оранжевая (Variable 1) имеет больший разброс.

## Вывод

Нет, образцы по выбранному параметру значимо не отличаются.

Хотя визуально на графике PCoA образцы группы Variable1 кажутся более "разбросанными" по краям, а Variable2 — более сгруппированными в центре, статистически на эти данные опираться нельзя.

При этом по количеству видов (все три параметра альфа-разнообразия) и общему составу бактерий (бета-разнообразие) группы очень похожи. Небольшие различия могут объясняться недостаточным количеством данных. Об отсутствии отличий говорят и результаты теста Уилкоксона.